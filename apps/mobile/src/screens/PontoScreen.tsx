...
export function PontoScreen() {
  ...
  const requestLocationAndCheckIn = async () => {
    if (!employeeId) {
      Alert.alert('Aviso', 'Nenhum funcionário vinculado. Use o primeiro da lista na API.');
      return;
    }
    try {
      ...
      Alert.alert('Sucesso', 'Check-in registrado com sua localização.');
    } catch (e: unknown) {
  ...
  const requestLocationAndCheckOut = async () => {
    ...
    try {
      ...
      Alert.alert('Sucesso', 'Check-out registrado com sua localização.');
    } catch (e: unknown) {
      Alert.alert('Erro', getApiErrorMessage(e));
    }
  };
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <Text style={styles.sectionTitle}>Registro de ponto (GPS obrigatório)</Text>
      ...
      {isLoading ? (
        ...
        history.map((r) => (
          <View key={r.id} style={styles.historyRow}>
            <Text style={styles.historyType}>{r.type === 'CHECKIN' ? 'Entrada' : 'Saída'}</Text>
