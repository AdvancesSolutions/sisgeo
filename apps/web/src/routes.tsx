...
function generateRoutesFromMenuItems(menuItems: MenuItem[]): React.ReactElement[] {
  return menuItems.flatMap((item: MenuItem) => {
    ...
    else if (path === "materials") routes.push(<Route key={item.id} path={path} element={<AdminOnly><Materials /></AdminOnly>} />);
    else if (path === "timeclock") routes.push(<Route key={item.id} path={path} element={<TimeClock />} />);
    else if (path === "reports") routes.push(<Route key={item.id} path={path} element={<AdminOnly><Reports /></AdminOnly>} />);
    else if (path === "audit") routes.push(<Route key={item.id} path={path} element={<AdminOnly><Audit /></AdminOnly>} />);
