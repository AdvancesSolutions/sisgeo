...
# Regra SPA: rewrite 200 para index.html (wildcard Amplify = <<*>>)
$RulesFile = Join-Path (Split-Path -Parent $PSScriptRoot) "scripts\amplify-custom-rules.json"
$CustomRulesJson = [System.IO.File]::ReadAllText($RulesFile).Trim()
...
if (-not $AppId -or $AppId -eq "None") {
    Write-Host "ERRO: App Amplify '$AppName' nao encontrado." -ForegroundColor Red
    exit 1
}
$AppId = $AppId.Trim()

Write-Host "Aplicando SPA rewrite no Amplify app $AppId..." -ForegroundColor Cyan
& $Awsexe amplify update-app --app-id $AppId --custom-rules $CustomRulesJson --region $AwsRegion 2>&1
